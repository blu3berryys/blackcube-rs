...
pub async fn bg(
    ...
    let content_type = Reader::new(Cursor::new(file_data)).with_guessed_format()?.format();

    if let Some(content_type) = content_type {
        if !ctx.data().content_types.valid_content_types.contains(&content_type) {
            ctx.say(format!("Image must be either {}. The image you uploaded is: {}.", ctx.data().content_types.concatenated_content_types, content_type.to_mime_type())).await?;
            return Ok(());
        }
    } else {
        ctx.say(format!("Image must be either {}. Your file is not an image.", ctx.data().content_types.concatenated_content_types)).await?;
        return Ok(());
    }

    let created_message_link = create_request_log_message(ctx, file.url).await?;
    ctx.say(format!("Created Request: {}", created_message_link)).await?;
    Ok(())
}
